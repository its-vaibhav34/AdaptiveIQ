...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await PaginationHelper.paginate(Question, query, { page, limit, skip, sort });

      return ResponseHelper.paginated(res, result.data, result.pagination, 'questions loaded successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const item = await Question.findOne({ _id: req.params.id, isDeleted: false });

      if (!item) {
        return ResponseHelper.notFound(res, 'Question');
      }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      return ResponseHelper.success(res, saved, 'Question created successfully', 201);
    } catch (error) {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      return ResponseHelper.success(res, updated, 'Question updated successfully');
    } catch (error) {
      if (error.name === 'ValidationError') {
        const errors = Object.values(error.errors).map(e => e.message);
        return ResponseHelper.validationError(res, errors);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const deleted = await Question.softDelete(req.params.id);

      if (!deleted) {
        return ResponseHelper.notFound(res, 'Question');
      }

      return ResponseHelper.success(res, deleted, 'Question deleted successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const items = await Question.insertMany(req.body.items);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const result = await Question.deleteMany({ _id: { $in: req.body.ids } });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const total = await Question.countDocuments({ isDeleted: false });
      const recent = await Question.find({ isDeleted: false }).sort({ createdAt: -1 }).limit(5);
