...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await PaginationHelper.paginate(Category, query, { page, limit, skip, sort });

      return ResponseHelper.paginated(res, result.data, result.pagination, 'category loaded successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      return ResponseHelper.success(res, saved, 'Category created successfully', 201);
    } catch (error) {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      return ResponseHelper.success(res, updated, 'Category updated successfully');
    } catch (error) {
      if (error.name === 'ValidationError') {
        const errors = Object.values(error.errors).map(e => e.message);
        return ResponseHelper.validationError(res, errors);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const deleted = await Category.softDelete(req.params.id);

      if (!deleted) {
        return ResponseHelper.notFound(res, 'Category');
      }

      return ResponseHelper.success(res, deleted, 'Category deleted successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const items = await Category.insertMany(req.body.items);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const recent = await Category.find({ isDeleted: false }).sort({ createdAt: -1 }).limit(5);
