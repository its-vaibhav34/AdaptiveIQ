...
import MousePo from '../models/MousePo.js';
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await PaginationHelper.paginate(MousePo, query, { page, limit, skip, sort });

      return ResponseHelper.paginated(res, result.data, result.pagination, 'mousePos loaded successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const item = await MousePo.findOne({ _id: req.params.id, isDeleted: false });

      if (!item) {
        return ResponseHelper.notFound(res, 'MousePo');
      }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const newItem = new MousePo(req.body);
      const saved = await newItem.save();

      return ResponseHelper.success(res, saved, 'MousePo created successfully', 201);
    } catch (error) {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      const updated = await MousePo.findByIdAndUpdate(
        req.params.id,
        req.body,
      ...
      return ResponseHelper.success(res, updated, 'MousePo updated successfully');
    } catch (error) {
      if (error.name === 'ValidationError') {
        const errors = Object.values(error.errors).map(e => e.message);
        return ResponseHelper.validationError(res, errors);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const deleted = await MousePo.softDelete(req.params.id);

      if (!deleted) {
        return ResponseHelper.notFound(res, 'MousePo');
      }

      return ResponseHelper.success(res, deleted, 'MousePo deleted successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const items = await MousePo.insertMany(req.body.items);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const result = await MousePo.deleteMany({ _id: { $in: req.body.ids } });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const total = await MousePo.countDocuments({ isDeleted: false });
      const recent = await MousePo.find({ isDeleted: false }).sort({ createdAt: -1 }).limit(5);
