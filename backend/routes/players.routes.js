...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Apply search filter
      if (req.query.search) {
        query = PaginationHelper.buildSearchQuery(req.query.search, ['score', 'avatar', 'username']);
      }
      ...
      const result = await PaginationHelper.paginate(Player, query, { page, limit, skip, sort });

      return ResponseHelper.paginated(res, result.data, result.pagination, 'players loaded successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const item = await Player.findOne({ _id: req.params.id, isDeleted: false });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const newItem = new Player(req.body);
      const saved = await newItem.save();

      return ResponseHelper.success(res, saved, 'Player created successfully', 201);
    } catch (error) {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      return ResponseHelper.success(res, updated, 'Player updated successfully');
    } catch (error) {
      if (error.name === 'ValidationError') {
        const errors = Object.values(error.errors).map(e => e.message);
        return ResponseHelper.validationError(res, errors);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const deleted = await Player.softDelete(req.params.id);

      if (!deleted) {
        return ResponseHelper.notFound(res, 'Player');
      }

      return ResponseHelper.success(res, deleted, 'Player deleted successfully');
    } catch (error) {
      next(error);
    }
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const items = await Player.insertMany(req.body.items);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const result = await Player.deleteMany({ _id: { $in: req.body.ids } });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const recent = await Player.find({ isDeleted: false }).sort({ createdAt: -1 }).limit(5);
